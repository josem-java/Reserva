...
class DashboardModel extends Query{
    ...
    public function rentasSemana(){
        $sql = "SELECT DATE_FORMAT(fecha_prestamo, '%Y-%m-%d') as fecha, SUM(cantidad * monto) as total
        FROM alquiler WHERE WEEK(fecha_prestamo) = WEEK(CURDATE())  -- Obtener la semana actual
