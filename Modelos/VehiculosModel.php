...
class VehiculosModel extends Query{
    ...
    public function getVehiculos(int $estado){
        ...
        tipos t ON v.id_tipo = t.id WHERE v.estado = $estado";
        $data = $this->selectAll($sql);
        return $data;
    }
    ...
    public function vehiculos(){
        $sql = "SELECT v.*, m.marca, t.tipo FROM vehiculos v INNER JOIN
        marcas m ON v.id_marca = m.id INNER JOIN
        tipos t ON v.id_tipo = t.id WHERE v.estado = 1 OR v.estado = 2";
        $data = $this->selectAll($sql);
        return $data;
    }
    ...
    $modelo, $kilometraje, $transmision, $asientos, $equipaje, $combustible, $imgNombre, $tipo, $marca){
        ...
        if (empty($existe)) {
            ...
            modelo, kilometraje, transmision, asientos, equipaje, combustible, foto, id_tipo, id_marca)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public function buscarVehiculo(string $valor){
        $sql = "SELECT v.*, t.tipo, m.marca FROM vehiculos v INNER JOIN
        tipos t ON t.id =  v.id_tipo INNER JOIN
        marcas m ON m.id = v.id_marca WHERE v.placa LIKE '%" . $valor . "%' AND v.estado = 1 OR t.tipo LIKE '%" . $valor . "%' AND v.estado = 1 OR m.marca LIKE '%" . $valor . "%'  AND v.estado = 1";
        $data = $this->selectAll($sql);
        return $data;
    }
}
